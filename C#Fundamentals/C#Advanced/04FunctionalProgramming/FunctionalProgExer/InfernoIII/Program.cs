    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((line = Console.ReadLine()) != "Forge")
            {
                ...
                if (command == "Exclude")
                {
                    var exludedForThisCommand = GetExcludedIndices(filter, parameter, originalNumbers);
                    history.Add($"{filter}:{parameter}", exludedForThisCommand);
                }
                else
                {
        ...
        private static readonly Func<string, int, int[], List<int>> GetExcludedIndices = (chosenFilter, wantedSum, numbers) =>
        {
